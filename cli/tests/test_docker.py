...
def test_missing_docker_binary(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("dockerctl._docker.shutil.which", lambda _: None)
    with pytest.raises(DockerError, match="docker CLI not found"):
        run("ps")


def test_success_streaming(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("dockerctl._docker.shutil.which", lambda _: "/usr/bin/docker")
    ...
    monkeypatch.setattr("dockerctl._docker.subprocess.run", fake_run)
